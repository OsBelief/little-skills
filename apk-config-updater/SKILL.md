...
此技能用于 PackageConfig 项目中自动更新 config 目录的 JSON 配置文件。通过提供 APK/包下载链接，自动解析包信息并更新到指定平台的配置文件中。
...
3. **后置验证**：更新后检查 git diff，自动回滚任何新增的包
4. **双重保险**：前置搜索 + 后置检查，确保不会意外新增

**为什么需要后置检查？**
- Gradle 任务 `genExtPkgInfo` 的设计缺陷会在所有指定配置文件中追加包（即使不存在）
- 即使前置搜索正确，执行时也可能因参数错误导致意外新增
- 后置检查作为最后一道防线，自动检测并回滚新增的配置
...
当用户执行 `/apk-config-updater <url>` 命令时，按以下步骤处理：
...
2. 将这些文件按渠道分组
3. 对每个渠道单独执行 Gradle 任务，只指定该渠道中包含该包的平台
...
for file in $(git diff --name-only); do
  ...
  if echo "$PKG_DIFF" | grep -E "^-.*(\"version\"|\"buildTime\"|\"md5")" > /dev/null; then
    ...
    echo "[✗ NEW] $file - 检测到包 $PKG_NAME 是新增，回滚整个文件..."
    git checkout "$file"
  fi
done
```
...
- 如果该片段中只有新增内容（以 `+` 开头），说明是新增包，必须回滚整个文件
...
### 检测到新增的包（自动回滚）
- **后置检查发现新增时，自动执行 `git checkout` 回滚**
- 向用户报告哪些文件被回滚
- 建议用户检查是否搜索到了正确的平台列表
- 如果确实需要新增，应手动操作并说明原因
